...
export default function ProductDetailScreen() {
  ...
  async function handlePlaceOrder() {
    ...
    if (!user?.system_user_id) {
      setOrderError('Unable to identify your merchant account. Please view your orders first.');
      return;
    }
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <View style={styles.row}>
        ...
        <Text style={styles.fieldValue}>{product.currency} {product.base_price.toFixed(2)} / {UNIT_LABELS[product.unit]}</Text>
      </View>
      <View style={styles.row}>
        <Text style={styles.fieldLabel}>Total Qty:</Text>
        <Text style={styles.fieldValue}>{product.total_quantity} {UNIT_LABELS[product.unit]}</Text>
      </View>
      <View style={styles.row}>
        <Text style={styles.fieldLabel}>Reserved:</Text>
        <Text style={styles.fieldValue}>{product.reserved_quantity} {UNIT_LABELS[product.unit]}</Text>
      </View>
      {product.quality_grade && (
        <View style={styles.row}>
          <Text style={styles.fieldLabel}>Grade:</Text>
          <Text style={styles.fieldValue}>{QUALITY_GRADE_LABELS[product.quality_grade]}</Text>
        </View>
      )}
      ...
      <View style={styles.row}>
        ...
        <Text style={[styles.fieldValue, styles.status]}>
          {product.status ? PRODUCT_STATUS_LABELS[product.status] : (product.is_active ? 'Active' : 'Inactive')}
        </Text>
      </View>
      ...
      {user?.role === 'MERCHANT' && (
        <View style={styles.orderSection}>
          ...
          {orderError ? <Text style={styles.fieldError}>{orderError}</Text> : null}
          <TouchableOpacity
...
const styles = StyleSheet.create({
  ...
  description: { fontSize: 15, color: '#424242', marginBottom: 12, lineHeight: 22 },
  ...
  orderTitle: { fontSize: 18, fontWeight: '700', color: '#1B5E20', marginBottom: 12 },
  input: {
    borderWidth: 1,
    borderColor: '#C8E6C9',
    borderRadius: 8,
