...
export default function ChatDetailScreen() {
  ...
  useEffect(() => { markAsRead(conversationId); }, [conversationId, markAsRead]);
...
function MessageBubble({ message, isMine }: { message: Message; isMine: boolean }) {
  return (
    <View style={[styles.bubbleWrap, isMine ? styles.bubbleWrapMine : styles.bubbleWrapTheirs]}>
      ...
      <View style={[styles.bubbleMeta, isMine ? styles.bubbleMetaMine : styles.bubbleMetaTheirs]}>
        <Text style={styles.bubbleTime}>{formatTime(message.timestamp)}</Text>
        {isMine && (
          <Text style={[styles.readTick, message.isRead && styles.readTickRead]}>
            {message.isRead ? '✓✓' : '✓'}
          </Text>
        )}
      </View>
...
const styles = StyleSheet.create({
  ...
  statusDot: {
    ...
    backgroundColor: '#25D366', marginRight: 6,
  },
  ...
  dateSepText: {
    fontSize: 12, color: '#666', backgroundColor: 'rgba(255,255,255,0.7)',
  ...
  bubbleMeta:       { flexDirection: 'row', alignItems: 'center', marginTop: 2, gap: 4 },
  ...
  input: {
    flex: 1, backgroundColor: '#F7F9F7', borderRadius: 22,
    paddingHorizontal: 16, paddingVertical: Platform.OS === 'ios' ? 10 : 8,
    fontSize: 15, color: '#0D1B0F', maxHeight: 100,
    borderWidth: 1, borderColor: '#E0E0E0',
  },
  sendBtn: {
    width: 44, height: 44, borderRadius: 22,
    backgroundColor: '#1A7A35', justifyContent: 'center', alignItems: 'center',
    shadowColor: '#1A7A35', shadowOpacity: 0.3, shadowRadius: 6,
    shadowOffset: { width: 0, height: 3 }, elevation: 3,
  },
  sendBtnDisabled: { backgroundColor: '#A5D6A7', shadowOpacity: 0, elevation: 0 },
