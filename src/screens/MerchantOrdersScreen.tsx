      ...
      <FlatList<OrderDTO>
        ...
        renderItem={({ item }) => (
          <OrderCard
            ...
            onPay={
              PAYABLE.includes(item.status) ? () => handlePay(item) : undefined
            }
            onPress={() =>
        ...
        ListEmptyComponent={
          <EmptyState
            message="No orders yet. Browse the marketplace to place one."
            emoji="📦"
          />
        }
        contentContainerStyle={
          orders.length === 0 ? styles.emptyFlex : styles.list
        }
      ...
      <View style={styles.cardTop}>
        <View style={styles.cardLeft}>
          ...
          <Text style={styles.orderMeta}>
            {new Date(order.created_at).toLocaleDateString()} · Qty{" "}
            {order.quantity}
          </Text>
        </View>
