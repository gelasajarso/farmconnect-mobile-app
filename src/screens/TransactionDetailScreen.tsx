    ...
    <SafeAreaView style={styles.safe}>
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.banner, { backgroundColor: statusColor }]}>
          ...
          <Text style={styles.bannerStatus}>{ORDER_STATUS_LABELS[order.status]}</Text>
          <Text style={styles.bannerAmount}>${order.total_price.toFixed(2)}</Text>
        </View>

        {/* Order Info */}
        <View style={styles.card}>
          <Text style={styles.cardTitle}>Order Details</Text>
          <DetailRow label="Order ID" value={`#${order.id.slice(-12).toUpperCase()}`} />
          <DetailRow label="Product ID" value={order.product_id} />
          <DetailRow label="Quantity" value={String(order.quantity)} />
          <DetailRow label="Unit Price" value={`$${order.unit_price.toFixed(2)}`} />
          <DetailRow label="Total Price" value={`$${order.total_price.toFixed(2)}`} bold />
...
const styles = StyleSheet.create({
  ...
  bannerLabel: { fontSize: 13, color: 'rgba(255,255,255,0.8)', fontWeight: '500' },
  bannerStatus: { fontSize: 20, fontWeight: '800', color: '#fff', marginTop: 4 },
  bannerAmount: { fontSize: 36, fontWeight: '800', color: '#fff', marginTop: 8 },
  card: {
    backgroundColor: '#fff', borderRadius: 12,
    marginHorizontal: 16, marginTop: 16, paddingHorizontal: 16,
  ...
  detailValue: { fontSize: 14, color: '#212121', fontWeight: '500', maxWidth: '55%', textAlign: 'right' },
  ...
  backBtn: {
    marginHorizontal: 16, marginTop: 20, backgroundColor: '#E3F2FD',
