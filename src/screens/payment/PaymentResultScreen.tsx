...
const PROVIDER_LABELS: Record<string, string> = {
  CHAPA: 'Chapa', TELEBIRR: 'Telebirr', BANK_TRANSFER: 'Bank Transfer',
};
...
export default function PaymentResultScreen() {
  ...
  const title   = isSuccess ? 'Payment Successful' : isPending ? 'Awaiting Verification' : 'Payment Failed';
    ...
    <SafeAreaView style={styles.safe}>
      <View style={styles.center}>
        ...
        <View style={styles.detailCard}>
          <DetailRow label="Provider"  value={PROVIDER_LABELS[provider] ?? provider} />
          <DetailRow label="Amount"    value={`ETB ${amount.toLocaleString('en-ET', { minimumFractionDigits: 2 })}`} bold />
          <DetailRow label="Order ID"  value={`#${order_id.slice(-8).toUpperCase()}`} />
        ...
        {isPending && (
          <View style={styles.pendingNote}>
            <Text style={styles.pendingNoteText}>
              📋 Your transfer reference has been submitted. Our team will verify your payment within 24 hours and update your order status.
            </Text>
          </View>
        )}
...
function DetailRow({ label, value, bold = false, valueColor }: {
  ...
  return (
    <View style={styles.detailRow}>
      ...
      <Text style={[styles.detailValue, bold && styles.detailValueBold, valueColor ? { color: valueColor } : null]}>
        {value}
      </Text>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  title:   { fontSize: 24, fontWeight: '800', color: '#0D1B0F', marginBottom: 8, textAlign: 'center' },
  message: { fontSize: 14, color: '#9E9E9E', textAlign: 'center', marginBottom: 24, lineHeight: 20 },

  detailCard: {
    width: '100%', backgroundColor: '#F7F9F7', borderRadius: 14,
  ...
  detailValue:     { fontSize: 13, color: '#0D1B0F', fontWeight: '600', maxWidth: '55%', textAlign: 'right' },
  detailValueBold: { fontSize: 15, fontWeight: '800' },

  pendingNote: {
    backgroundColor: '#FFF3E0', borderRadius: 12, padding: 14,
    marginBottom: 20, borderLeftWidth: 3, borderLeftColor: '#E65100',
  },
