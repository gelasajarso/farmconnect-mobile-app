...
import {
  ...
  StyleSheet, SafeAreaView, ActivityIndicator, Platform, Alert,
} from 'react-native';
import { useNavigation, useRoute } from '@react-navigation/native';
import type { StackNavigationProp, RouteProp } from '@react-navigation/stack';
import type { MerchantStackParamList } from '../../navigation/types';
import { getBankAccounts, submitBankTransfer } from '../../services/payment.service';
...
export default function BankTransferScreen() {
  ...
  async function handleSubmit() {
    ...
    if (!ref) { setRefError('Transaction reference is required.'); return; }
    if (ref.length < 6) { setRefError('Reference must be at least 6 characters.'); return; }
      ...
      <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.amountCard}>
          ...
          <Text style={styles.amountValue}>
            ETB {paymentParams.amount.toLocaleString('en-ET', { minimumFractionDigits: 2 })}
          </Text>
        ...
        {accounts.map(acc => (
          <TouchableOpacity
            ...
            style={[
              styles.accountCard,
              selectedAccount.account_number === acc.account_number && styles.accountCardSelected,
            ]}
          ...
          >
            ...
            <View style={[
              styles.radio,
              selectedAccount.account_number === acc.account_number && styles.radioSelected,
            ]}>
              {selectedAccount.account_number === acc.account_number && <View style={styles.radioDot} />}
            </View>
          </TouchableOpacity>
        ))}
        ...
        <View style={styles.instructionCard}>
          ...
          <Text style={styles.instructionStep}>1. Transfer the exact amount to the account above</Text>
          <Text style={styles.instructionStep}>2. Note your transaction reference number</Text>
          <Text style={styles.instructionStep}>3. Enter the reference below and submit</Text>
          <Text style={styles.instructionStep}>4. Our team will verify within 24 hours</Text>
        </View>

        {/* Reference input */}
        <Text style={styles.sectionTitle}>Transaction Reference</Text>
        <View style={[styles.inputWrap, refError ? styles.inputWrapError : null]}>
          <TextInput
            style={styles.input}
            value={reference}
...
const styles = StyleSheet.create({
  ...
  amountCard: {
    ...
    shadowColor: '#1565C0', shadowOpacity: 0.3, shadowRadius: 10,
    shadowOffset: { width: 0, height: 4 }, elevation: 4,
  },
  amountLabel: { fontSize: 12, color: 'rgba(255,255,255,0.75)', fontWeight: '600', textTransform: 'uppercase', letterSpacing: 0.5 },
  amountValue: { fontSize: 32, fontWeight: '800', color: '#fff', marginVertical: 4, letterSpacing: -1 },
  amountSub:   { fontSize: 13, color: 'rgba(255,255,255,0.7)' },

  sectionTitle: { fontSize: 14, fontWeight: '700', color: '#0D1B0F', marginBottom: 10, letterSpacing: -0.2 },
  ...
  accountLeft:   { flex: 1 },
  bankName:      { fontSize: 14, fontWeight: '700', color: '#0D1B0F', marginBottom: 3 },
  accountName:   { fontSize: 13, color: '#555', marginBottom: 2 },
  accountNumber: { fontSize: 15, fontWeight: '800', color: '#1565C0', letterSpacing: 1, marginBottom: 2 },
  ...
  instructionTitle: { fontSize: 13, fontWeight: '700', color: '#1565C0', marginBottom: 8 },
  instructionStep:  { fontSize: 13, color: '#555', marginBottom: 4, lineHeight: 18 },
  ...
  input:      { fontSize: 16, color: '#0D1B0F', fontWeight: '600', letterSpacing: 0.5 },
  fieldError: { fontSize: 12, color: '#B71C1C', marginBottom: 14, fontWeight: '500' },

  submitBtn: {
    backgroundColor: '#1565C0', borderRadius: 14,
    paddingVertical: 17, alignItems: 'center', marginTop: 8,
    shadowColor: '#1565C0', shadowOpacity: 0.35, shadowRadius: 12,
    shadowOffset: { width: 0, height: 5 }, elevation: 5,
  },
  submitBtnDisabled: { backgroundColor: '#90CAF9', shadowOpacity: 0, elevation: 0 },
