...
const STEPS: Record<string, string[]> = {
  CHAPA:    ['Initializing payment…', 'Opening Chapa checkout…', 'Verifying payment…'],
  TELEBIRR: ['Sending USSD push…', 'Waiting for approval…', 'Confirming payment…'],
};
    ...
    async function run() {
      try {
        if (provider === 'CHAPA') {
          ...
          if (cancelled) return;
          if (initRes.status !== 'success') throw new Error('Payment initialization failed');
          ...
          if (cancelled) return;
          const status: PaymentStatus = verifyRes.data.status === 'success' ? 'SUCCESS' : 'FAILED';
          ...
          if (cancelled) return;
          if (initRes.status !== 'success') throw new Error('Telebirr initialization failed');
          ...
          if (cancelled) return;
          animateStep(2);
          const statusRes = await pollTelebirrStatus(initRes.data.transaction_id);

          if (cancelled) return;
          const status: PaymentStatus = statusRes.status === 'SUCCESS' ? 'SUCCESS' : 'FAILED';
          navigation.replace('PaymentResult', {
            status,
            provider,
            amount: paymentParams.amount,
            ...
            message: status === 'SUCCESS' ? 'Payment confirmed' : 'Payment failed or timed out',
          });
        }
      } catch (err: any) {
        if (cancelled) return;
...
const styles = StyleSheet.create({
  ...
  provider: { fontSize: 13, color: '#9E9E9E', fontWeight: '600', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 8 },
  amount:   { fontSize: 36, fontWeight: '800', color: '#0D1B0F', letterSpacing: -1, marginBottom: 24 },

  step: { fontSize: 16, color: '#1A7A35', fontWeight: '600', textAlign: 'center', marginBottom: 20 },
